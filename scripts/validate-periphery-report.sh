...
if [[ $# -ne 4 ]]; then
    echo "Usage: $0 <baseline> <report> <scan-log> <scan-status>" >&2
    exit 2
fi
...
if [[ ! $SCAN_STATUS =~ ^[0-9]+$ ]]; then
    echo "::error::Periphery scan status must be a non-negative integer" >&2
    exit 1
fi

if [[ $SCAN_STATUS -ne 0 ]]; then
    echo "::error::Periphery scan failed with exit code $SCAN_STATUS" >&2
    [[ -f $SCAN_LOG_PATH ]] && cat "$SCAN_LOG_PATH" >&2
    [[ -f $REPORT_PATH ]] && cat "$REPORT_PATH" >&2
    exit 1
fi

if [[ ! -f $BASELINE_PATH || ! -f $REPORT_PATH || ! -f $SCAN_LOG_PATH ]]; then
    echo "::error::Periphery baseline, report, and scan log files must exist" >&2
    exit 1
fi
...
if ! grep -Eq '^\[xcode:project\] Loading .*/GenreUpdater\.xcodeproj$' "$SCAN_LOG_PATH" \
    ...
    echo "::error::Periphery scan log does not prove analysis of the GenreUpdater app target" >&2
    cat "$SCAN_LOG_PATH" >&2
    exit 1
fi
...
if [[ $BASELINE_ENTRY_COUNT -ne 2 || ! $UNUSED_BASELINE =~ ^[0-9]+$ || ! $ACCESSIBILITY_BASELINE =~ ^[0-9]+$ ]]; then
    echo "::error::Periphery baseline must contain exactly two non-negative integer categories" >&2
    exit 1
fi
...
if [[ $WARNING_COUNT -eq 0 && $BASELINE_WARNING_COUNT -gt 0 ]]; then
    echo "::error::Scan reported zero warnings against nonzero baselines; treating as a failed scan" >&2
    cat "$REPORT_PATH" >&2
    exit 1
fi

if [[ $WARNING_COUNT -ne $KNOWN_WARNING_COUNT ]]; then
    echo "::error::Periphery reported an unclassified warning category" >&2
    exit 1
fi

if [[ $UNUSED_COUNT -gt $UNUSED_BASELINE ]]; then
    echo "::error::Unused declarations rose from ${UNUSED_BASELINE} to ${UNUSED_COUNT}" >&2
    exit 1
fi

if [[ $ACCESSIBILITY_COUNT -gt $ACCESSIBILITY_BASELINE ]]; then
    echo "::error::Redundant public accessibility rose from ${ACCESSIBILITY_BASELINE} to ${ACCESSIBILITY_COUNT}" >&2
    exit 1
fi
